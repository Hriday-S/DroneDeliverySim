...
if pc_prev != NUM_CUSTOMERS:
    ...
    st.rerun()
# Use local customer count alias
...
def _rand_grid():
    ...
    grid_codes = np.full((GRID_RES, GRID_RES), -1, int)
    for k in farm_idx:
...
def _fetch_grid(url):
    ...
    for x,y,v in zip(gx,gy,data.ravel()): cell_vals[y][x].append(v)
        ...
        for d,pos in enumerate(self.dpos):
            ...
            while rem>eps and self.assign[d]:
                ...
                if dist<eps:
                    ...
                    continue
                stepv=min(rem,dist); frac=stepv/dist; pos=(pos[0]+vec[0]*frac,pos[1]+vec[1]*frac); rem-=stepv
            self.dpos[d]=pos
...
# Warning if no farms
if not np.any(st.session_state.grid_codes==CROP_CODE): st.warning(f"No {crop_label} farms found. Start disabled.")
...
# Plot grid
grid_fig,ax=plt.subplots(figsize=(6,6)); ax.set_xlim(0,GRID_SIZE); ax.set_ylim(0,GRID_SIZE); ax.set_aspect('equal'); ax.invert_yaxis()
for i in range(GRID_RES):
    for j in range(GRID_RES):
